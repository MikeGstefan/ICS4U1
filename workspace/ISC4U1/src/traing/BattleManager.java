...
public class BattleManager {
	...
	public static void main(String[] args) {
		int[] dead = new int[NUM_PLAYERS];
		int[] avgDead = new int[NUM_PLAYERS];
		int genNum = 0;
		GenData[] newGen = {
			new GenData(3,3,4, 4.0  ,0.9, 12.0, 15.0, 30.0), 
			new GenData(8,1,1, 5.0  ,0.1, 27.0, 13.0, 9.0), 
			new GenData(4,4,2, 3.0  ,0.8, 11.0, 14.0, 5.0), 
			new GenData(1,1,8, 3.3  ,0.2, 4.0,  30.0, 19.0),
			new GenData(1,8,1, 8.0  ,0.7, 8.0,  12.5, 10.0),
			new GenData(6,3,1, 2.0  ,0.6, 10.0, 10.0, 10.0),
			new GenData(1,2,7, 11.0 ,0.5, 12.0, 13.0, 60.0),
			new GenData(6,1,3, 2.8  ,0.3, 25.3, 10.0, 0.1),
			new GenData(1,4,5, 2.0  ,0.4, 14.0, 0.1, 9.0)
		};
		boolean first = true;
		int temp = 0, index = 0;;
		GenData output = GenData.mix(newGen[0], newGen[1]);
		System.out.format("%d, %d, %d, %f, %f, %f, %f, %f",  output.attack, output.defence, output.numMoves, output.MOVE_DIST_SCALER, output.OPP_DIST_SCALER, output.DIFF_SCALER, output.DIFF_ACCUARACY_SCALER, output.HEALTH_WEIGHT);
		while(true){
			if(!first){
				for(int i = 0; i< NUM_PLAYERS; i++){
					avgDead[i] = (int)Math.round((double)avgDead[i] /10.0);
				}
				...
				for(int i = 0; i < NUM_PLAYERS; i++){
					...
					avgDead[index] = avgDead[i];
					avgDead[i] = temp;
				}

				
				System.out.print(avgDead[0]);
				GenData.modify(newGen, avgDead, genNum);
				genNum++;

			}
		// boolean genNew = false;
		first = false;
		Arrays.fill(avgDead,0);
		for(int l = 0; l <= 10; l++){
			health = new int[NUM_PLAYERS];             // an array to keep track of the FighterRobots' health (index position refers to the robot ID#)
			energy = new int[NUM_PLAYERS];             // an array to keep track of the FighterRobots' energy (index position refers to the robot ID#)
			numFights = new int[NUM_PLAYERS];  // an array to keep track of the number of fights, a penalty will apply if robot does not initiate a fight in 5 rounds.
			stats = new PlayerStats[NUM_PLAYERS];
			dead = new int[NUM_PLAYERS];
			num_lost = 0;
		// genNew = true;
			createCity();
			createPlayers(newGen);
				...
				for (int i = 0; i < NUM_PLAYERS; i++)
				{
					if (num_lost < NUM_PLAYERS - 1)
					{
						...
						if (health[i] > 0)
						{
							...
							doTurn(i, player[i].takeTurn(energy[i], playerData), dead);
			...
			for(int i = 0; i < NUM_PLAYERS; i++){
				...
				if(!found){
					dead[NUM_PLAYERS-1] = i;
					// avgDead[NUM_PLAYERS-1] += NUM_PLAYERS-1;
					break;
				}
			...
			// for(int i = 0; i < NUM_PLAYERS; i++){ 
			// 	System.out.format("death order: %d\n",dead[i]);
			// 	System.out.format("avg dead: %d\n",avgDead[i]);
	 ...
	 */
	private static void createPlayers(GenData[] generate)
	{
		Random generator = new Random();
		// create robots for the arena 
		player[0] = new StefanFighterRobot(arena,generator.nextInt(HEIGHT),generator.nextInt(WIDTH),Direction.NORTH, 0, MAX_HEALTH, generate[0]);
		// player[1] = new TestFighterRobot(arena,1,1,Direction.NORTH, 1, MAX_HEALTH);
		player[1] = new	StefanFighterRobot(arena,generator.nextInt(HEIGHT),generator.nextInt(WIDTH),Direction.NORTH, 1, MAX_HEALTH, generate[1]);
		player[2] = new StefanFighterRobot(arena,generator.nextInt(HEIGHT),generator.nextInt(WIDTH),Direction.NORTH, 2, MAX_HEALTH, generate[2]);
		player[3] = new StefanFighterRobot(arena,generator.nextInt(HEIGHT),generator.nextInt(WIDTH),Direction.NORTH, 3, MAX_HEALTH, generate[3]);
		player[4] = new StefanFighterRobot(arena,generator.nextInt(HEIGHT),generator.nextInt(WIDTH),Direction.NORTH, 4, MAX_HEALTH, generate[4]);
		player[5] = new StefanFighterRobot(arena,generator.nextInt(HEIGHT),generator.nextInt(WIDTH),Direction.NORTH, 5, MAX_HEALTH, generate[5]);
		player[6] = new StefanFighterRobot(arena,generator.nextInt(HEIGHT),generator.nextInt(WIDTH),Direction.NORTH, 6, MAX_HEALTH, generate[6]);
		player[7] = new StefanFighterRobot(arena,generator.nextInt(HEIGHT),generator.nextInt(WIDTH),Direction.NORTH, 7, MAX_HEALTH, generate[7]);
		player[8] = new StefanFighterRobot(arena,generator.nextInt(HEIGHT),generator.nextInt(WIDTH),Direction.NORTH, 8, MAX_HEALTH, generate[8]);
	 ...
	 */
	private static void doTurn(int attackerID, TurnRequest request, int[] dead)
	{
						...
						for (int n = 0; n < request.getNumRounds(); n ++)
						{
							if (energy[attackerID] > 0 && health[attackerID] > 0 && health[defenderID] > 0)
							{
								...
								else
								{
									...
									// System.out.format("%d, %d\n", player[defenderID].getDefence(), defenderID);
